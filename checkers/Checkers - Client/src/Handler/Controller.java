...
public class Controller implements Runnable {
	...
	public void run() {
		...
		try {
			...
			if(isOver){
		        String winner = "";
		        if (boardPanel.countCheckers(Checkers.PLAYER_ONE.getValue()) == 0) {
		            winner = "Player Two";
		        } else if (boardPanel.countCheckers(Checkers.PLAYER_TWO.getValue()) == 0) {
		            winner = "Player One";
		        }
		        JOptionPane.showMessageDialog(null, winner + " is the winner. Game is over",
	...
	public void move(Square from, Square to){
		...
		if (boardPanel.countCheckers(Checkers.PLAYER_ONE.getValue()) == 0) {
	        isOver = true;
	        // Send a message to the server that player two has won
	        try {
	            toServer.writeInt(Checkers.YOU_WIN.getValue());
	        } catch (IOException e) {
	            System.out.println("Sending failed");
	        }
	    } else if (boardPanel.countCheckers(Checkers.PLAYER_TWO.getValue()) == 0) {
	        isOver = true;
	        // Send a message to the server that player one has won
	        try {
	            toServer.writeInt(Checkers.YOU_WIN.getValue());
	        } catch (IOException e) {
	            System.out.println("Sending failed");
